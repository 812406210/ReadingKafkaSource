...
public class NetworkClient implements KafkaClient {
    ...
    public boolean ready(Node node, long now) {
        //1、已经建立好连接，底层的NIO Channel是ok的
        //2、inFlightRequests没有满5个，此时就可以针对这个broker去发送一个请求过去
        if (node.isEmpty())
    ...
    private void initiateConnect(Node node, long now) {
        ...
        try {
            ...
            //底层NIO,selector建立socket连接,并设置socket发送和接受的缓冲区
            //selector的组件进行连接：NIO建立连接，它其实就是在底层初始化一个SocketChannel 发起一个连接请求，就会把SocketChannel给注册到selector上面
            //，让selector监听它建立连接的事件。如果Broker返回响应说可以建立连接，selector就回告诉你，通过一个API调用完成底层的网络连接，Tcp三次握手，双方都有一个socket
