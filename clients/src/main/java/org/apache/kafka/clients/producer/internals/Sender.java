...
public class Sender implements Runnable {
    ...
    void run(long now) {
        ...
        if (!result.unknownLeaderTopics.isEmpty()) {
            ...
            //如果有一些topic的元数据没有拉取到，就会尝试拉取元数据
            this.metadata.requestUpdate();
        }

        // remove any nodes we aren't ready to send to
        //检查一下是否准备好可以向那么broker发送数据，如果此时还没跟某个broker建立好连接，就必须在这里建立好，以便发送数据过去
        ...
        //创建需要发送的数据包(batch)，并根据broker进行分区聚合，包含集群信息，发送的broker节点，请求数据大小以及时间
        Map<Integer, List<RecordBatch>> batches = this.accumulator.drain(cluster,
                                                                         result.readyNodes,
                                                                         this.maxRequestSize,
                                                                         now);
        //guaranteeMessageOrder表示消息是否被订阅过
        ...
        //创建发送数据请求:对每个broker都创建一个ClientRequest,包括了多个batch，就是在这个broker上的多个leader Partition所对应的Batch聚合起来组成一个ClientRequest，形成一个请求，发送到broker上
        ...
        //发送batch消息数据：通过NetWorkClient走底层网络通信，把每个broker的ClientRequest给发送过去就可以了，
        //poll方法，它是负责实际的进行网络IO通信操作的一个核心方法，负责发送数据出去，也包括读取响应数据回来
