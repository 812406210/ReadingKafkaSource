...
final class ClusterConnectionStates {
    ...
    public boolean canConnect(String id, long now) {
        //先找到broker id 对应的一个连接状态，如果此时这个连接状态是null,就说明之前从来没有建立连接，此时直接返回true,说明可以跟这个broker建立连接；
        //否则如果连接状态已经存在，但是状态是断开的，而且上一次跟这个broker尝试建立连接的时间到现在，已经超过了重试的时间，默认为1000ms
