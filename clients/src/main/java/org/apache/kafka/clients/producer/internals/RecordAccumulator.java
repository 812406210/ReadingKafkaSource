...
public final class RecordAccumulator {
    ...
    public RecordAppendResult append(TopicPartition tp,
        ...
        try {
            ...
            synchronized (dq) {
                ...
                //tryAppend() 如果batch是存在的，就会将消息放入队列最近一个batch中，然后直接返回；
            ...
            //为batch分配内存空间，默认为16kb;如果你的消息大于16kb的话，就会使用你的消息大小来分配一块内存空间
            //实际生产中，request.max.siez,batch.size是要结合实际进行调优的
    ...
    //在往这个Deque里写入数据的时候放一个个的Batch的时候，也会加锁，在从Deque里读取数据的时候也会加锁的
    //如果此时判断出来这个Batch是可以发送出去的，此时就会将这个Batch对应的那个Partition的leader broker给放入到一个set集合中
            ...
            synchronized (deque) {
                if (leader == null && !deque.isEmpty()) {
                    ...
                    if (batch != null) {
                        ...
                        //batch等待时间：当前时间减去上一次发送这个batch的时间
                        long waitedTimeMs = nowMs - batch.lastAttemptMs;
                        //这个batch从创建开始算起，最多等待多久就必须发送出去，如果在重试阶段，这个时间就是重试间隔，如果在非重试阶段，就是linger.ms(100ms)
                        long timeToWaitMs = backingOff ? retryBackoffMs : lingerMs;
                        long timeLeftMs = Math.max(timeToWaitMs - waitedTimeMs, 0);
                        boolean full = deque.size() > 1 || batch.records.isFull();
                        //当前batch已经等待的时间 >= Batch最多只能等待的时间 ，则设置为过期batch
                        boolean expired = waitedTimeMs >= timeToWaitMs;
                        //综合上述条件，这个batch是否需要发送出去，如果batch已满必须的发送；如果batch没有写满但是已经过期也必须要发送出去；如果batch没有写满也没有过期但是内存耗尽也必须发送
                        ...
                        if (sendable && !backingOff) {
                            ...
                            //如果还有batch没有达到发送的条件，则会更新nextReadyCheckDelayMs时间
